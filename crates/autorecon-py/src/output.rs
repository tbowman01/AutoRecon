//! Output handler using native filesystem operations for Python
...
/// Output handler that uses direct filesystem operations
#[derive(Clone)]
pub struct PyOutputHandler {
    /// Verbosity level for logging
    verbosity: u8,
}

impl PyOutputHandler {
    /// Create a new Python output handler
    ///
    /// # Arguments
    /// * `verbosity` - Verbosity level (0 = quiet, 1 = normal, 2 = verbose)
    pub fn new(verbosity: u8) -> Self {
        PyOutputHandler { verbosity }
    }
